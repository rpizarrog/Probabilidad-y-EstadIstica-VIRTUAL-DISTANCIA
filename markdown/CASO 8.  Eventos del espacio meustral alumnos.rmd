---
title: "CASO 8.  Eventos del espacio meustral alumnos"
...
-   Se crea un espacio meustral de alumno llamando una función que se encuentra en la dirección de github.com
...
![](images/experimento%20espacio%20muestral%20eventos%20y%20probabilidades.png)
...
![](images/dado%20%20DEL%201%20AL%206-01.jpg){width="424"}
...
El operador *%in%* evalúa si los valores de S están en el vector *c(2,4,6)*. Con la función *paste()* se muestra el mensaje.

La función *which()* devuelve la posición o *índice (index) de un elemento dentro de un vector*, ejemplo de *which*(). Se tienen 12 números:
...
```{r}
numeros <- c(3,4,5,6,2,1,4,2,5,4,6,5)
numeros
n <- length(numeros)
posiciones <- which(numeros < 4)
paste("Las posiciones o índices (index)")
numeros[posiciones]
```

Con esa misma función *which()* se construyen los eventos.
...
![](images/alumnos%20universitarios-01.jpg){width="419"}

Crear espacio muestral alumnos

$$
S = \text{{x | x son alumnos inscritos en una institución educativa de nivel superior}}
$$
